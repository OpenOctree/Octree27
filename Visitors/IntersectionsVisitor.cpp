...
namespace Clobscode
{
    ...
    void IntersectionsVisitor::setFaces(list<unsigned long> &faces){
        this->faces = &faces;
    }
    ...
    bool IntersectionsVisitor::visit(Octant *o) {
        ...
        if (select_faces) //checkIntersections(TriMesh &mesh,list<unsigned long> &faces,vector<Point3D> &coords);
        {
            if (faces == NULL || coords == NULL)
                throw std::runtime_error(std::string("Calling wrong IntersectionsVisitor!"));
            list<unsigned long> &intersected_faces = o->intersected_faces;

            list<unsigned long>::iterator f_iter;
    ...
    bool IntersectionsVisitor::intersectsTriangle(SurfTriangle &st, vector<Point3D> &input_pts, const Point3D &pmin,
        ...
        vector<unsigned long> face = st.getPoints();

        unsigned long n_pts = face.size();

        vector<unsigned long> sides(n_pts,0);
        ...
        unsigned long all_and = sides[0];
        for (unsigned long i=1; i<n_pts; i++) {
        ...
        for (unsigned long i=0; i<n_pts; i++) {
            unsigned long next = (i+1)%n_pts;
            unsigned long result_or = sides[i]|sides[next];
            unsigned long result_xor = sides[i]^sides[next];
            if (result_or == 48) {
                //48 <> 11 00 00
                return true;
            }
    ...
    bool IntersectionsVisitor::edgeTriangleIntersection(SurfTriangle &st,
        ...
        for (unsigned long i=0; i<oct_edges.size(); i++) {
    ...
    unsigned long IntersectionsVisitor::computePosition(const Point3D &p, const Point3D &pmin, const Point3D &pmax) {
