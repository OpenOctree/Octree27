...
{	
	...
	void RefinementSurfaceRegion::rotate(GeometricTransform &gt) {
		...
		for (unsigned long i=0; i<tm.getPoints().size(); i++){
			gt.apply(tm.getPoints()[i]);
        }
		
		vector<vector<unsigned long> > faces (tm.getFaces().size(), vector<unsigned long>(3, 0));
	...
	bool RefinementSurfaceRegion::intersectsOctant(vector<MeshPoint> &points, Octant &oct)
    {
        ...
        for (unsigned long i=0; i<tmpts.size(); i++) {
                                                   ...
                                                   Point3D &seg_p1, Point3D &seg_p2)
	{
		...
		vector<unsigned long> sides(2,0);
		...
		//unsigned long all_and = sides[0] & sides[1];
        ...
        unsigned long result_or = sides[0]|sides[1];
        unsigned long result_xor = sides[0]^sides[1];
        if (result_or == 48) {
            //48 <> 11 00 00
            return true;
        }
	...
	unsigned long RefinementSurfaceRegion::computePosition(const Point3D &p,
                                                          const Point3D &pmin,
                                                          const Point3D &pmax){
