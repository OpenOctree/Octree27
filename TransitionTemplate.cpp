...
Uint TransitionTemplate::getNewElements(const UintVec &hex_idxs,
   ...
   if (findPattern()) {
      ...
      UintMap points;
      // Gets map (rotated_point, init_point)
      ...
      //pt.clear(); // INFO VS crashes w/ this
...
void TransitionTemplate::vectors(VectorTable &v) {
   ...
   UintMap points;
   // Returns a map of (rotated_point, init_point)
   m_cube->getCurrentTransitionPoints(points);
...
void TransitionTemplate::getEdgesToRefine(VectorTable* to_refine) {
   Uint ma = m_cube->getEdgeTransitionPointsMask();
   VectorTable edges;
   factory->getTemplateEdges(ma, edges);
   UintMap points;
   m_cube->getCurrentTransitionPoints(points);
   ...
   for (vector<Uint>& e : edges) {
      ...
      if (p == points.end()) { // Edge is not refined, but it should be.
         ...
         // px -> p0  == ix_local_original -> ix_global
         Uint p0 = m_NodesMap.find(px)->second.External;
